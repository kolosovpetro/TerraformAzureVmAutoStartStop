...
if (!$VMs)
{
    Write-Output "No VM with state schedule management tag ($TagName) have been found in the provided scope (rg : $ResourceGroupName)"
    exit
}
...
foreach ($VM in $VMs)
{
    ...
    Write-Output "Current time is $CurrentTime, using $TimeZone timezone."
